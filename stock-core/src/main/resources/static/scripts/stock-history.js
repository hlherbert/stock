...
(function () {
    ...
    const stockmetas = queryStockMetas();
    ...
    jQuery(document).ready(function () {
        ...
        inputSelectCode = document.querySelector('#input-select-code');
        inputSelectCode.addEventListener('input', onCodeChange);
        inputCode = document.querySelector('#input-code');

        // 起始时间 结束时间
        inputStart = document.querySelector('#input-start');
        inputEnd = document.querySelector('#input-end');
        inputStart.addEventListener('input', onStartEndChange);
        inputEnd.addEventListener('input', onStartEndChange);

        // 股票名称
        inputName = document.querySelector('#input-name');

        // 查询按钮
        btnQuery = document.querySelector('#btn-query');
        btnQuery.addEventListener('click', onBtnQueryClick);

        // 图表
        chartHistory = echarts.init(document.querySelector('#chart-history'));
    ...
    function onBtnQueryClick(e) {
        ...
        updateStockHistoryChart(chartHistory, code, start, end);
    }

    // 验证表单
    function validateForm() {
        let v1 = validate(inputSelectCode, document.querySelector('#code-error'), "股票代码非法! 000001");
        let v2 = validate(inputStart, document.querySelector('#start-error'), "日期格式非法! 20000101");
        let v3 = validate(inputEnd, document.querySelector('#end-error'), "日期格式非法! 20300101");
        let v = v1 && v2 && v3;
        if (v) {
    ...
    // 通用验证函数
    function validate(input, error, errMsg) {
    ...
    // 查询历史数据
    function queryStockHistory(code, start, end, callback) {
    ...
    // 更新图表数据
    function updateStockHistoryChart(chart, code, start, end) {
